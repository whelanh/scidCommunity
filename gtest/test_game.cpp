...
TEST(Test_Game, clone) {
  for (auto filename : {gameUTF8, gameLatin1, gameLatin1Conv}) {
    ...
    ASSERT_TRUE(std::equal(board, board + 66, clone->currentPos()->GetBoard()));
    ...
    clone->ResetPgnStyle(PGN_STYLE_TAGS | PGN_STYLE_VARS | PGN_STYLE_COMMENTS |
                         PGN_STYLE_SCIDFLAGS);
...
TEST(Test_Game, currentPosUCI_fen) {
  ...
  const std::pair<unsigned, const char *> expected[] = {
      ...
      {5,
       "position fen 8/8/8/8/2p5/1k1p4/p4N2/2K5 w - - 0 198 moves f2d3 a2a1r"},
      {6, "position fen 8/8/8/8/2p5/1k1p4/p4N2/2K5 w - - 0 198 moves f2d3 "
          "a2a1r c1d2"},
      {7, "position fen 8/8/8/8/2p5/1k1p4/p4N2/2K5 w - - 0 198 moves f2d3 "
          "a2a1r c1d2 c4d3"},
      {8,
       "position fen 8/8/8/8/2p5/1k1p4/p4N2/2K5 w - - 0 198 moves c1d2 a2a1q"},
      {9, "position fen 8/8/8/8/2p5/1k1p4/p4N2/2K5 w - - 0 198 moves c1d2 "
          "a2a1q d2e3"},
      {10, "position fen 8/8/8/8/2p5/1k1p4/p4N2/2K5 w - - 0 198 moves c1d2 "
           "a2a1q d2e3 a1e1"}
...
TEST(Test_Game, illegalSCID4_Castling) {
  ...
  auto check =
      make_invalid(10, // replace 5...h5 with O-O
  ...
  auto twice =
      make_invalid(10, // replace 5.a4 with O-O
  ...
  // No rook
  auto no_rook =
      make_invalid(9, // replace 2.c4 with O-O-O
  ...
  auto occ_king =
      make_invalid(0, // replace 2.Nf3 with a null move
  ...
  auto occ_rook =
      make_invalid(0, // replace 3...Be7 with a null move
