...
const byte NAG_GoodMove = 1, NAG_PoorMove = 2, NAG_ExcellentMove = 3,
           NAG_Blunder = 4, NAG_InterestingMove = 5, NAG_DubiousMove = 6,
           NAG_OnlyMove = 8, // new
    NAG_Equal = 10, NAG_Unclear = 13, NAG_WhiteSlight = 14,
           NAG_BlackSlight = 15, NAG_WhiteClear = 16, NAG_BlackClear = 17,
           NAG_WhiteDecisive = 18, NAG_BlackDecisive = 19,
           NAG_WhiteCrushing = 20, NAG_BlackCrushing = 21,
           NAG_ZugZwang = 22,           // new
...
class Game {
  ...
  uint HtmlStyle;        // HTML diagram style, see DumpHtmlBoard method in
                         // position.cpp.
...
public:
  ...
  std::string &assignTagValue(std::string_view tag, Args &&...args) {
    ...
    if (!value) {
      auto it =
          std::find_if(extraTags_.begin(), extraTags_.end(),
  ...
  // PGN conversion
  void WriteComment(TextBuffer *tb, const char *preStr, const char *comment,
                    const char *postStr);
  errorT WriteMoveList(TextBuffer *tb, moveT *oldCurrentMove, bool printMoveNum,
                       bool inComment);
  ...
  bool MaterialMatch(bool PromotionsFlag, ByteBuffer &buf, byte *min, byte *max,
                     patternT *ptn, size_t ptn_size, int minPly, int maxPly,
