...
namespace {
[[maybe_unused]] bool valid_sqlist(pieceT *begin, size_t n, pieceT *board) {
...
inline void Position::AddHash(pieceT p, squareT sq) {
  HASH(Hash, p, sq);
...
inline void Position::UnHash(pieceT p, squareT sq) {
  UNHASH(Hash, p, sq);
  if (piece_Type(p) == PAWN) {
...
inline void Position::AddToBoard(pieceT p, squareT sq) {
...
inline void Position::RemoveFromBoard(pieceT p, squareT sq) {
  ASSERT(Board[sq] == p);
  Board[sq] = EMPTY;
  NumOnRank[p][square_Rank(sq)]--;
  NumOnFyle[p][square_Fyle(sq)]--;
...
struct sqDir_Init {
  sqDir_Init() {
    ...
    // of squares.
    squareT i, j;
    directionT dirArray[] = {UP,       DOWN,      LEFT,       RIGHT,   UP_LEFT,
                             UP_RIGHT, DOWN_LEFT, DOWN_RIGHT, NULL_DIR};
...
//
inline void Position::CalcPinsDir(directionT dir, pieceT attacker) {
...
bool Position::under_attack(squareT target_sq, squareT captured_sq,
  ...
  const auto lpawn_atk =
      square_Move(target_sq, enemy == WHITE ? DOWN_LEFT : UP_LEFT);
  const auto rpawn_atk =
      square_Move(target_sq, enemy == WHITE ? DOWN_RIGHT : UP_RIGHT);
  ...
  for (size_t i = 1, n = GetCount(enemy); i < n; ++i) {
    ...
    if (piece_type == KNIGHT) {
      ...
      if (movegen::attack_slider(piece_sq, target_sq, piece_type, not_empty))
...
//
void Position::GenKingMoves(MoveList *mlist, genMovesT genType) {
...
//
inline void Position::AddPromotions(MoveList *mlist, squareT from,
                                    squareT dest) {
...
inline bool Position::IsValidEnPassant(squareT from, squareT to) {
...
void Position::setCastling(colorT col, squareT rsq) {
  ...
  Castling |= 1u << idx;
  const auto std_rsq =
      (dir == QSIDE) ? square_Relative(col, A1) : square_Relative(col, H1);
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// Position::StdStart():
//      Set up the standard chess starting position. For performance the data is
...
void Position::GenerateMoves(MoveList *mlist, pieceT pieceType,
  ...
  if (pieceType != EMPTY) {
    ...
    mask = (1 << KING) | (1 << QUEEN) | (1 << ROOK) | (1 << BISHOP) |
           (1 << KNIGHT) | (1 << PAWN);
  }
...
static bool xray_check(Position const &pos, squareT from, squareT to) {
  const auto [atk_piece, atk_sq] =
      movegen::opens_ray(from, to, pos.GetKingSquare(),
...
//
bool Position::IsKingInCheck(simpleMoveT const &sm) {
...
uint Position::Mobility(pieceT p, colorT color, squareT from) {
...
errorT Position::MakeCoordMoves(const char *moves, size_t moveslen,
  ...
  while (auto len = std::find_if(moves, end, is_space) - moves) {
    ...
    if (toSAN) {
      char san[8];
      MakeSANString(&sm, san, moves != end ? SAN_CHECKTEST : SAN_MATETEST);
      if (WhiteToMove()) {
...
void Position::DumpHtmlBoard(DString *dstr, uint style, const char *dir,
                             bool flip) {
