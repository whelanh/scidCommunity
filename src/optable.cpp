...
inline bool posHasAdvancedPawn(Position *pos, colorT c) {
  if (c == WHITE) {
    return (pos->RankCount(WP, RANK_5) > 0 || pos->RankCount(WP, RANK_6) > 0 ||
            pos->RankCount(WP, RANK_7) > 0);
  }
  return (pos->RankCount(BP, RANK_4) > 0 || pos->RankCount(BP, RANK_3) > 0 ||
...
inline bool posHasKPawnStorm(Position *pos, colorT c) {
  ...
  if (c == WHITE) {
    return (bd[G2] != WP && bd[H2] != WP && bd[G3] != WP && bd[H3] != WP &&
            (pos->FyleCount(WP, G_FYLE) > 0 || pos->FyleCount(WP, H_FYLE) > 0));
  }
  return (bd[G7] != BP && bd[H7] != BP && bd[G6] != BP && bd[H6] != BP &&
          (pos->FyleCount(BP, G_FYLE) > 0 || pos->FyleCount(BP, H_FYLE) > 0));
}
...
void OpLine::Init(Game *g, const IndexEntry *ie, gamenumT gameNum,
...
void OpLine::PrintNote(DString *dstr, uint movenum, uint start, uint format) {
  ...
  if (format == OPTABLE_HTML || format == OPTABLE_CText) {
    preFirstMove = "<b>";
    postFirstMove = "</b>";
  }
...
void OpLine::PrintSummary(DString *dstr, uint format, bool fullDate,
                          bool nmoves) {
...
void OpTable::Init(const char *type, Game *g, PBook *ebook) {
...
void OpTable::SetFormat(const char *str) { Format = FormatFromStr(str); }
...
uint OpTable::PercentScore() {
  ...
  percent = percent * 500;
  uint total =
      Results[RESULT_White] + Results[RESULT_Draw] + Results[RESULT_Black];
  if (total > 0) {
...
uint OpTable::NoteCount(uint note) {
  uint count = 0;
...
uint OpTable::NoteScore(uint note) {
  uint count = 0;
  uint score = 0;
  for (uint n = 0; n < NumTableLines; n++) {
    if (Line[n]->NoteNumber == note) {
...
void OpTable::BestGames(DString *dstr, uint count, const char *rtype) {
  enum { BEST_White, BEST_Black, BEST_AvgElo, BEST_Oldest, BEST_Newest };
  int rt = BEST_AvgElo;
  switch (rtype[0]) {
  case 'W':
  ...
  for (uint c = 1; c <= count; c++) {
    ...
    if (Line[bestIndex]->NoteNumber != 0) {
      if (Format == OPTABLE_HTML) {
        ...
        dstr->Append(">[", Line[bestIndex]->NoteNumber, "]</go></red>");
      } else {
  ...
  for (uint n = 1; n <= count; n++) {
    ...
    if (found) {
      ...
      if (maxYear == 0) {
        ...
        sprintf(tempStr, "%4u%s%4u", minYear, inRange, maxYear);
      }
      dstr->Append(nextCell, " ", tempStr);
